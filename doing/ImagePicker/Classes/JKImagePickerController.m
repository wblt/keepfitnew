...
@interface JKImagePickerController ()<JKAssetsGroupsViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,JKAssetsViewCellDelegate,JKPhotoBrowserDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIGestureRecognizerDelegate>
...
- (void)setUpProperties
{
    ...
    [preBtn setTitleColor:UIColorFromRGB(0x22ac38) forState:UIControlStateNormal];
    [preBtn setTitleColor:UIColorFromRGB(0x22ac38) forState:UIControlStateHighlighted];
    [preBtn setTitleColor:UIColorFromRGB(0x22ac38) forState:UIControlStateDisabled];
...
- (void)loadAllAssetsForGroups
{
    ...
    self.assetsArray = [[assets reverseObjectEnumerator] allObjects];
...
- (void)setShowsCancelButton:(BOOL)showsCancelButton{
    ...
    if (showsCancelButton) {
        ...
        [cancelBtn setTitleColor:UIColorFromRGB(0xc9caca) forState:UIControlStateNormal];
        [cancelBtn setTitleColor:UIColorFromRGB(0xc9caca) forState:UIControlStateHighlighted];
...
- (void)finishPhotoDidSelected
{
    if ([_delegate respondsToSelector:@selector(imagePickerController:didSelectAssets:isSource:)])
    {
        [_delegate imagePickerController:self
                         didSelectAssets:self.aryAssest
...
- (void)photoBrowser:(JKPhotoBrowser *)photoBrowser didSelectAtIndex:(NSInteger)index
{
    ...
    NSURL *assetURL = [asset valueForProperty:ALAssetPropertyAssetURL];
    [self addAssetsObject:assetURL withAlasset:asset];
...
- (void)photoBrowser:(JKPhotoBrowser *)photoBrowser didDeselectAtIndex:(NSInteger)index
{
    ALAsset *asset = self.assetsArray[index];
    NSURL *assetURL = [asset valueForProperty:ALAssetPropertyAssetURL];
    [self removeAssetsObject:assetURL withAlasset:asset];
    [self resetFinishFrame];
    [self.collectionView reloadData];
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    */
    ...
    if([self.delegate respondsToSelector:@selector(imagePickerControllerDidCancel:withImage:)])
    {
        [self.delegate imagePickerControllerDidCancel:self withImage:image];
    }
...
- (void)didSelectItemAssetsViewCell:(JKAssetsViewCell *)assetsCell
{
    ...
    if (validate) {
        ...
        [self addAssetsObject:assetURL withAlasset:assetsCell.asset];
        [self resetFinishFrame];
...
{
    NSURL *assetURL = [assetsCell.asset valueForProperty:ALAssetPropertyAssetURL];
    [self removeAssetsObject:assetURL withAlasset:assetsCell.asset];
    [self resetFinishFrame];
...
- (void)removeAssetsObject:(NSURL *)assetURL withAlasset:(ALAsset *)alasset
{
    ...
    //2015-04-26
    if([self.aryAssest containsObject:alasset])
    {
        [self.aryAssest removeObject:alasset];
    }
}

- (void)addAssetsObject:(NSURL *)assetURL withAlasset:(ALAsset *)alasset
{
    ...
    //2015-04-26
    [self.aryAssest addObject:alasset];
}
...
- (NSMutableArray *)aryAssest
{
    if (!_aryAssest) {
        _aryAssest = [NSMutableArray arrayWithCapacity:0];
    }
    return _aryAssest;
}
